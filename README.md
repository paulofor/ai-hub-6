...
3. Instale o Maven localmente para executar comandos do backend (`mvn test`, `mvn clean package`). A imagem do sandbox já vem com Maven, JDK, Docker CLI, plugin Docker Compose v2 (`docker compose`), AWS CLI, GitHub CLI (`gh`), `actionlint`, `ffmpeg`, `ffprobe`, `sandbox-media-player`, Playwright, `@playwright/test` e Chromium headless pré-instalados; se precisar configurar a sua máquina, siga [este passo a passo](docs/maven-setup.md).

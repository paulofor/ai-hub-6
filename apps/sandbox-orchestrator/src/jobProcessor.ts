...
export class SandboxJobProcessor implements JobProcessor {
  ...
  private async cloneRepository(job: SandboxJob, repoPath: string, cloneUrl: string): Promise<void> {
    ...
    const startedAt = new Date(cloneStartedAtMs).toISOString();
    ...
    try {
      ...
      // Preserve the time spent even when clone itself fails and the job terminates early.
      job.cloneDurationMs = Math.max(0, Date.now() - cloneStartedAtMs);
    }
    this.recordDownload(job, {
      source: 'git',
      url: safeUrl,
  ...
  private buildBrowserTestingInstruction(): string {
...
${stderr}`);
  ...
  private async runRunnerPreflight(job: SandboxJob, repoPath: string): Promise<void> {
    ...
    if (await this.isCommandAvailable('sandbox-media-player')) {
      mediaTools.push('sandbox-media-player');
    }

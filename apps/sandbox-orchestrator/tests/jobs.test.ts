...
test('executa CHATGPT_CODEX_MKT via Codex App Server com instruções de marketing', async () => {
  ...
  try {
    ...
    assert.ok(input?.[0]?.text?.includes('viewport, user agent, touch e deviceScaleFactor de celular'));
    assert.ok(input?.[0]?.text?.includes('Para vídeos, áudio e animações em mobile'));
    assert.ok(input?.[0]?.text?.includes('O ffmpeg e o ffprobe estão disponíveis para o modelo pelos comandos ffmpeg e ffprobe'));
...
test('inclui checklist de ambiente OK no prompt inicial do runner', async () => {
  ...
  try {
    ...
    assert.match(promptText, /melhor opção de simulador de celular disponível na sandbox é o Playwright com emulação mobile do Chromium/i);
    assert.match(promptText, /devices\["iPhone 15 Pro"\]/i);
    assert.match(promptText, /devices\["Pixel 7"\]/i);
    assert.match(promptText, /viewport, user agent, touch e deviceScaleFactor de celular/i);
    assert.match(promptText, /Para vídeos, áudio e animações em mobile/i);

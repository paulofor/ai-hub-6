...
| `HEYGEN_TOKEN_HOST_DIR` | Diretório físico do host montado como segredo somente leitura em `/run/secrets/heygen-token`; quando contém o arquivo `heygen_api_key`, o `docker-compose` exporta seu conteúdo como `HEYGEN_API_KEY` antes de iniciar o runner/Codex App Server. | `/root/infra/heygen-token` |
...
Os tokens Luma, Kling e HeyGen devem ficar fora do repositório nos arquivos `/root/infra/luma-token/luma_api_key`, `/root/infra/kling-token/kling_api_key` e `/root/infra/heygen-token/heygen_api_key`. Quando os arquivos existem, o `sandbox-orchestrator` exporta `LUMA_API_KEY`, `KLING_API_KEY` e `HEYGEN_API_KEY` para os comandos do modelo e para o Codex App Server; nunca registre os valores dessas variáveis em logs, respostas ou arquivos.
...
O runner informa também que Playwright, `@playwright/test` e Chromium headless estão disponíveis para validações visuais. A imagem usa `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` e reaproveita o Chromium do sistema em `/usr/bin/chromium`, exposto por `CHROME_BIN`, `CHROMIUM_BIN`, `PUPPETEER_EXECUTABLE_PATH` e `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH`; `NODE_PATH=/usr/local/lib/node_modules` permite carregar os pacotes globais em scripts Node executados pelo modelo. A melhor opção de simulador de celular na sandbox é Playwright com emulação mobile do Chromium, usando `devices["iPhone 15 Pro"]` ou `devices["Pixel 7"]` do `@playwright/test`; o modelo deve usar esse recurso sempre que precisar testar uma URL como usuário de celular, com viewport, user agent, touch e `deviceScaleFactor` mobile. Para vídeos, áudio e animações em mobile, combine essa emulação com `sandbox-media-player`, `ffmpeg` e `ffprobe` para validar reprodução, duração, frames, sincronia, controles nativos e comportamento visual no layout mobile.

O runner informa ainda que `ffmpeg` e `ffprobe` estão disponíveis para manipulação e inspeção de vídeos. Use `ffmpeg` para converter, cortar, extrair áudio, gerar thumbnails e sintetizar mídias de teste, e use `ffprobe` para conferir metadados, codecs, resolução, duração, streams e integridade básica de arquivos de vídeo recebidos ou gerados na sandbox. Para avaliação perceptual, o helper `sandbox-media-player <arquivo-video-ou-audio> [saida.html]` gera uma página HTML local com player nativo; abra o HTML com Chromium/Playwright para reproduzir a mídia e avaliar naturalidade da pronúncia, sincronização labial, cortes e qualidade perceptual além da validação técnica.

...
```text
operation=device_user_code
response={device_auth_id=[redacted], user_code=[redacted], interval=5, expires_at=2026-06-22T15:48:43.986309+00:00}
```
...
```text
operation=authorization_code_exchange
response={access_token=[redacted], token_type=[redacted], expires_in=864000, scope=openid profile email offline_access, id_token=[redacted], earliest_refresh_at=1782920070, refresh_token=[redacted], oai_is=[redacted]}
```

### 5. Criação da CodexRequest 717
...
CodexRequest 717 salvo, enviando para sandbox se aplicável
Enviando CodexRequest 717 para sandbox com jobId 06394ad9-538c-4c35-985b-6c21c5a86106 e branch padrão main
```
...
```text
operation=oauth_token_refresh
response={access_token=[redacted], token_type=[redacted], expires_in=864000, scope=openid profile email offline_access, id_token=[redacted], earliest_refresh_at=1782920116, refresh_token=[redacted], oai_is=[redacted]}
```
...
```text
CodexRequest 717 será executado sem token OAuth válido de conta conectada
```
...
Na 717 essas duas falhas não aparecem. O refresh usa `client_id=app_EMoamEEZ73f0CkXaXp7hrann`, não envia `client_secret` e não envia `organization_id`.

### Comparação com 714

A 717 reproduz o mesmo resultado funcional da 714: device login público e refresh bem-sucedidos, mas `id_token` sem `organization_id`. A diferença é que na 717 o backend já bloqueia o token exchange antes de receber `401 invalid_subject_token`, evitando uma chamada que a OpenAI já rejeitou anteriormente.

### Comparação com 715

Na 715 o refresh falhou porque `organization_id` voltou a ser enviado no corpo do refresh e a OpenAI respondeu `400 unknown_parameter`. Na 717 isso foi corrigido: o refresh foi aceito. Portanto a correção da 715 resolveu o erro de payload, mas não resolveu a ausência de organização no `id_token` de sessões device.

### Comparação com 716

A 716 e a 717 têm a mesma causa final: a sessão ativa ainda era device login público. O ajuste posterior à 716 fez a UI preferir login browser quando `oauthConfigured=true`, mas a investigação operacional da 717 indicou que a produção ainda tinha `HUB_ACCOUNT_OAUTH_CLIENT_ID=paulofore`, valor inválido que fazia o backend não expor OAuth browser como pronto e mantinha a UI no fallback device.

## Conclusão

A execução 717 não falhou por `invalid_client` nem por `organization_id` no refresh. Ela falhou porque a sessão usada para executar Codex ainda foi originada pelo device login público, que, nos logs observados das execuções 714, 716 e 717, não retorna `organization_id` no `id_token`.
